...
@click.option(
    "--bg-class-name",
    default="background",
    help="The name of the background class utilized by the input model.",
)
@click.option(
    "--no-trace",
    is_flag=True,
...
def detect_v2(
    ...
    Output categories in the COCO file generated by this tool will have IDs
    that represent the index of the model inference output for that category.
    The category section will **not** include a "background" category (or
    otherwise specified by --bg-class-name).
    This may cause the category IDs to **not** start at 0.
